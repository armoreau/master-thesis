...
options = Odeoptions() #Create an option with default value.
...
tspan = np.linspace(0,1000,25)
y0 = np.array([2])

options = Odeoptions()
options.odeset('RelTol',np.array([1e-9]))
options.odeset('AbsTol',np.ones([len(y0)])*1e-10)
